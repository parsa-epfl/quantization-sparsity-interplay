...
idx = np.arange(12)
plt.plot(idx, delta_scores[:, 0], linestyle='dashed')
plt.plot(idx, delta_scores[:, 1], linestyle='dashed')
plt.plot(idx, delta_scores[:, 2], linestyle='dashed')
plt.scatter(idx, delta_scores[:, 0], label="1st_quartile")
plt.scatter(idx, delta_scores[:, 1], label="median")
plt.scatter(idx, delta_scores[:, 2], label="3rd_quartile")
plt.legend()
plt.xlabel("layer_idx")
plt.ylabel("|weights_dense - weights_sparse|")
plt.title("Absolute difference between dense and sparse attention weights: 50 samples")
